/**
 * yswang add to support 'crossFrame' config.
 * WdatePicker.js?crossFrame=true
 */
var _currScript = function(){if(document.currentScript){return document.currentScript.src;}
var stack;try{a.b.c();}catch(e){stack=e.stack;if(!stack&&window.opera){stack=(Strin(e).match(/of linked script \S+/g)||[]).join(" ");}}
if(stack){stack=stack.split(/[@ ]/g).pop();stack=stack[0]=="("?stack.slice(1,-1):stack;return stack.replace(/(:\d+)?:\d+$/i,"");}
var nodes=document.getElementsByTagName("script");for(var i=0,node;node=nodes[i++];){if(node.readyState==="interactive"){return node.className=node.src;}}};
var crossFrame = _currScript().match(/(\?|&)crossFrame=(true|yes|1)(&|$)/gi) !== null ? true : false;
/*
